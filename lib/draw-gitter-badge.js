...
function calculateParams (options, roominfo) {
  var margin        = options.mini ? 1 : MARGIN
    , roomName      = 'gitter.im/' + roominfo.name
    , usersText     = (roominfo.users || 0) + ' user'
                        + (roominfo.users === 1 ? '' : 's')
                        + ' now'
    , updatedText   = 'last message ' + moment(roominfo.updated).fromNow()
...
function draw (options, roominfo) {
  var params = calculateParams(options, roominfo)
